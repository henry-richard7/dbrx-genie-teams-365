...
def message_handler(mock_database):
    with patch("handlers.message_handler.GenieListHandler"), \
        ...
        handler.genie_list_handler.handle_list_spaces = AsyncMock(return_value="list response")
        return handler
...
async def test_process_message_list_spaces(message_handler, mock_turn_context, monkeypatch):
    # Arrange
    ...
    # Assert
    message_handler.genie_list_handler.handle_list_spaces.assert_called_once_with(user_id="user_123")
    mock_turn_context.send_activity.assert_any_call("list response")

async def test_process_message_genie_question_with_selection(message_handler, mock_turn_context, mock_database):
    # Arrange
    mock_turn_context.activity.value = None
...
async def test_process_message_genie_question_no_selection(message_handler, mock_turn_context, mock_database):
    # Arrange
    mock_turn_context.activity.value = None
    mock_turn_context.activity.text = "what are the sales?"
...
async def test_handle_card_action_select_space(mock_database, mock_turn_context):
    with patch("handlers.message_handler.GenieListHandler"), \
        ...
        await handler.handle_card_action(
            mock_turn_context, "user_123", {"action": "select_space", "space_id": "s1", "space_name": "Space 1"}
        )
...
async def test_handle_space_selection(mock_database, mock_turn_context):
    with patch("handlers.message_handler.GenieListHandler"), \
        ...
        mock_database.update_user_selection.assert_called_once_with(
            user_id="user_123", space_id="s1", space_name="Space 1", conversation_id=None
        )
        mock_turn_context.send_activity.assert_called_once_with(
